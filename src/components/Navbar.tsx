...
const Dropdown = memo(({
  ...
  return (
    ...
    >
      <button
        onClick={handleClick}
        className={`flex items-center space-x-1 ${isMobile ? 'w-full py-2 text-left' : 'py-4'
          } text-base font-medium transition-all duration-500 ${isActiveDropdown ? 'text-primary' : 'text-gray-700 hover:text-primary'
          }`}
      >
        ...
        >
          ...
          {item.dropdownItems?.map(dropdownItem => (
            <Link
              ...
              className={`block pl-4 py-2 text-[1rem] transition-all duration-500 ${location.pathname === dropdownItem.path ? 'text-primary' : 'text-gray-700 hover:text-primary'
                }`}
              onClick={() => {
                setIsOpen(false);
                onItemClick?.();
        ...
        >
          ...
          {item.dropdownItems?.map(dropdownItem => (
            <Link
              key={dropdownItem.path}
              to={dropdownItem.path}
              className={`block px-4 py-2 text-[1rem] transition-all duration-300 ${location.pathname === dropdownItem.path
                  ? 'text-primary mx-2'
...
const Navbar = () => {
  ...
  const renderNavItem = useCallback((item: NavItem, isMobile = false) => {
    ...
    >
      <div className='max-w-6xl mx-auto flex items-center justify-between px-6 lg:px-0'>
        ...
        <div className='hidden lg:flex items-center space-x-6'>
          <Link
            to='/projects'
            className={`flex items-center text-base font-medium transition-all duration-500 group ${location.pathname === '/projects' ? 'text-primary' : 'text-gray-700 hover:text-primary'
              }`}
          >
      ...
      >
        ...
        >
          ...
          <div
            className={`
            w-full pt-2 border-t border-muted/40 transform transition-all duration-500 delay-[${navItems.length * 100
              }ms]
            ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'}
          `}
          >
